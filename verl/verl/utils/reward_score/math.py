...
def compute_score(solution_str, ground_truth) -> dict:
    ...
    Returns:
        dict: {"score": float, "acc": bool, "pred": str}
              与 DAPO 格式保持一致，避免 batch 混合时长度不匹配
    """
    retval = 0.0
    pred = None
    try:
        ...
        if string_in_last_boxed is not None:
            ...
            pred = answer  # 保存提取的预测答案
            
            # 🔧 从 ground_truth 中提取标准答案
            # MATH 数据集的 ground_truth 可能包含完整的解答过程
            # 需要提取其中的答案部分（支持 \boxed{} 和 #### 两种格式）
            gt_answer = ground_truth
            
            # 方法1：尝试提取 #### 后面的答案（GSM8K 格式）
            if isinstance(ground_truth, str) and "####" in ground_truth:
                gt_answer = ground_truth.split("####")[-1].strip()
            # 方法2：尝试提取 \boxed{} 中的答案（MATH 格式）
            elif isinstance(ground_truth, str) and "\\boxed" in ground_truth:
                gt_boxed = last_boxed_only_string(ground_truth)
                if gt_boxed is not None:
                    gt_answer = remove_boxed(gt_boxed)
            
            # 比较提取的答案
            if is_equiv(answer, gt_answer):
                retval = 1.0
