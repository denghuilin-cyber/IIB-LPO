...
def verify(
    ...
    # 🔧 优先尝试提取 \boxed{} 格式的答案
    # 支持多种可能的反斜杠格式：\boxed{}, \\boxed{}, \\\boxed{} 等
    import re
    
    # 尝试用正则表达式直接提取 boxed 内容
    # 匹配 \boxed{...} 或 \\boxed{...} 等各种转义情况
    boxed_patterns = [
        r'\\+boxed\{([^}]+)\}',  # 匹配一个或多个反斜杠 + boxed{内容}
    ]
    ...
    for pattern in boxed_patterns:
        ...
        if matches:
            # 取最后一个匹配（通常是最终答案）
            pred = normalize_final_answer(matches[-1])
            break
    ...
    if pred is None:
        ...
        if boxed_answer is not None:
            try:
                pred = normalize_final_answer(remove_boxed(boxed_answer))
            except Exception as e:
                pass  # 如果提取失败，继续尝试 Answer: 格式
    
    # 如果还是没有提取到答案，回退到 Answer: 格式
    if pred is None:
        correct, pred = is_correct_minerva(solution_str, answer)
        return correct, pred
    
    # 标准化 ground truth 并比较
    gt = normalize_final_answer(answer)
    return (pred == gt), pred
