    ...
    def augment(self, gen_batch: DataProto) -> DataProto:
        ...
        for orig_idx in range(original_batch_size):
            ...
            for rep_idx in range(self.num_repeats):
                ...
                # Decode original prompt to text（现在只包含有效token，没有padding）
                # 🔧 修复：使用 skip_special_tokens=False 保留完整的 chat template 结构
                # 因为 multi_dataset_with_cot.py 已经构建了完整的 prompt，包含所有特殊标记
                ...
                if skip_this_prompt:
                    if rep_idx == 0:  # Log only once per prompt
                        logger.warning(f"No COT example found for prompt {orig_idx}, using original prompt without augmentation")
                    augmented_text = prompt_text
                else:
                    # Get COT example for this repetition
                    cot_text = cot_examples_for_prompt[rep_idx]
                    
                    # ⭐ 步骤 1 & 2: 正确地将 COT 插入到 prompt 中
                    # 策略：将 COT 插入到 <|im_start|>assistant 之后，<|im_end|> 之前
                    # 原始 prompt 格式：
                    ...
                    # 查找 <|im_start|>assistant 的位置
                    assistant_marker = "<|im_start|>assistant\n"
                    if assistant_marker in prompt_text:
                        # 在 assistant 标记后插入 COT
                        parts = prompt_text.split(assistant_marker, 1)
                        if self.add_separator:
                            cot_section = f"Here is an example:\n{cot_text}\n\nNow solve the problem:\n"
                        else:
                            cot_section = cot_text
                        potential_text = parts[0] + assistant_marker + cot_section
                    else:
                        # 如果找不到 assistant 标记，回退到简单拼接
                        logger.warning(f"样本 {orig_idx} 中找不到 '<|im_start|>assistant\\n'，回退到简单拼接")
                        if self.add_separator:
