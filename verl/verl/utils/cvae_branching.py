...
class CVAEBranchingManager:
    ...
    def __init__(
        ...
        # 🔒 冻结 CVAE 模型参数
        for param in self.cvae_model.parameters():
            param.requires_grad = False
        logger.info("  ✅ CVAE 模型参数已冻结（不参与训练）")
    ...
    def create_z_projection_layer(self, hidden_dim: int) -> nn.Module:
        ...
        if hidden_dim not in self.z_projection_layers:
            ...
            # 🆕 设置 requires_grad=True，允许训练
            for param in projection.parameters():
                param.requires_grad = True
            self.z_projection_layers[hidden_dim] = projection
            logger.info(f"🔧 创建了可训练的 z 投影层: {self.latent_dim} -> {hidden_dim}")
        
        return self.z_projection_layers[hidden_dim]
    
    def get_projection_parameters(self):
        """
        🆕 获取所有投影层的参数（用于创建 optimizer）
        
        Returns:
            所有投影层参数的 iterator
        """
        params = []
        for projection in self.z_projection_layers.values():
            params.extend(list(projection.parameters()))
        logger.info(f"[CVAE Projection] 获取投影层参数: {len(params)} 个参数组")
        for i, p in enumerate(params):
            logger.debug(f"  参数 {i}: shape={p.shape}, requires_grad={p.requires_grad}")
        return params
    
    def save_projection_layers(self, save_path: str):
        """
        🆕 保存投影层的权重
        ...
        """
        state_dict = {
            dim: projection.state_dict() 
            for dim, projection in self.z_projection_layers.items()
        }
        torch.save(state_dict, save_path)
        logger.info(f"✅ 投影层已保存到: {save_path}")
        logger.info(f"  保存了 {len(state_dict)} 个投影层")
    
    def load_projection_layers(self, load_path: str):
        """
        🆕 加载投影层的权重
        ...
        if not os.path.exists(load_path):
            logger.warning(f"⚠️ 投影层权重文件不存在: {load_path}")
            return False
        
        state_dict = torch.load(load_path, map_location=self.device)
        
        for dim, projection_state in state_dict.items():
            # 创建投影层（如果不存在）
            self.create_z_projection_layer(dim)
            # 加载权重
            self.z_projection_layers[dim].load_state_dict(projection_state)
        
        logger.info(f"✅ 投影层已从 {load_path} 加载")
        logger.info(f"  加载了 {len(state_dict)} 个投影层")
        return True
    ...
    def project_z_to_hidden(
        ...
        """
        将 z 投影到 hidden_dim 空间（用于 INPUT Fusion 或 PSA Fusion）
        
        这个方法用于 ForwardContext 方案，直接返回 z_proj tensor
        
        Args:
            z: 潜在向量 [1, latent_dim]
            hidden_dim: 模型的 hidden_size（如 4096）
            mode: 投影模式，用于日志输出（"INPUT" 或 "PSA"）
        
        Returns:
            z_proj: 投影后的向量 [1, hidden_dim]
        """
        logger.info(f"[{mode} Fusion] 投影 z 到 hidden 空间...")
        logger.info(f"  z 形状: {z.shape}")
        ...
        if hidden_dim not in self.z_projection_layers:
            ...
            nn.init.normal_(projection.weight, mean=0.0, std=0.01)
            self.z_projection_layers[hidden_dim] = projection
            logger.info(f"  创建了投影层: {self.latent_dim} -> {hidden_dim}")
        
        # 投影 z 到 hidden 空间
        z_proj_layer = self.z_projection_layers[hidden_dim]
        z_proj = z_proj_layer(z)  # [1, hidden_dim]
        
        logger.info(f"  z {z.shape} -> z_proj {z_proj.shape}")
        logger.info(f"  z_proj 统计: 均值={z_proj.mean().item():.6f}, "
                   f"标准差={z_proj.std().item():.6f}")
        logger.info(f"✅ {mode} 投影完成")
    ...
    def set_z_injection_for_psa_fusion(
        self,
        z: torch.Tensor,
        hidden_dim: int,
        num_layers: int,
        injection_layers: Union[int, str] = 4
    ):
        """
        🆕 通过 vLLM ForwardContext 设置 PSA Fusion 注入配置
        
        ⚠️ 互斥性保证：不能与 INPUT/SOFTMAX Fusion 同时使用
        
        Args:
            z: 潜在向量 [1, latent_dim]
            hidden_dim: 模型的 hidden_size
            num_layers: 模型的总层数
            injection_layers: 注入层数配置（4, 8, "all"）
        ...
        logger.info(f"[PSA Fusion] 🔧 准备设置 z_injection_config（ForwardContext 方案）")
        
        # 1. 投影 z 到 hidden_dim
        z_proj_psa = self.project_z_to_hidden(z, hidden_dim, mode="PSA")
        
        # 2. 确定要注入的层
        if injection_layers == "all":
            psa_injection_layers = list(range(num_layers))
            logger.info(f"  注入所有 {num_layers} 层")
        elif isinstance(injection_layers, int):
            # 最后 N 层
            psa_injection_layers = list(range(num_layers - injection_layers, num_layers))
            logger.info(f"  注入最后 {injection_layers} 层: {psa_injection_layers}")
        else:
            # 默认最后 4 层
            psa_injection_layers = list(range(num_layers - 4, num_layers))
            logger.warning(f"  未知的 injection_layers: {injection_layers}，使用默认最后 4 层")
        
        # 3. 创建 ZInjectionConfig（只设置 PSA 相关字段）
        config = ZInjectionConfig(
            mode="psa",
            z_proj_input=None,  # 🔒 明确设置为 None
            z_proj_psa=z_proj_psa,
            psa_injection_layers=psa_injection_layers,
            z_proj_vocab=None  # 🔒 明确设置为 None
        )
        
        # 4. 验证互斥性
        validate_z_injection_config(config)
        
        logger.info(f"✅ [PSA Fusion] z_injection_config 已创建（INPUT/SOFTMAX 已禁用）")
        logger.info(f"  mode: {config.mode}")
        logger.info(f"  z_proj_psa: {config.z_proj_psa.shape if config.z_proj_psa is not None else None}")
        logger.info(f"  psa_injection_layers: {config.psa_injection_layers}")
        logger.info(f"  z_proj_input: {config.z_proj_input}")
        logger.info(f"  z_proj_vocab: {config.z_proj_vocab}")
    ...
    def get_projection_parameters(self):
        ...
        Returns:
            List[torch.nn.Parameter]: 所有投影层的参数列表
        """
        params = []
        for projection_layer in self.z_projection_layers.values():
            params.extend(list(projection_layer.parameters()))
        return params
    
    def get_projection_layers_dict(self):
        """
        获取投影层字典（用于保存/加载）
        
        Returns:
            Dict[int, nn.Module]: 投影层字典 {hidden_dim: projection_layer}
        """
        return self.z_projection_layers
    
    def set_projection_layers_trainable(self, trainable: bool = True):
        """
        设置投影层是否可训练
        
        Args:
            trainable: True=可训练, False=冻结
        """
        for projection_layer in self.z_projection_layers.values():
        ...
        logger.info(f"[CVAE] 投影层参数已设置为: {status}")
        logger.info(f"  投影层数量: {len(self.z_projection_layers)}")
        total_params = sum(p.numel() for p in self.get_projection_parameters())
    ...
    def save_projection_layers(self, save_path: str):
        ...
        Args:
            save_path: 保存路径（.pth 文件）
        """
        state_dict = {}
        for hidden_dim, projection_layer in self.z_projection_layers.items():
            state_dict[f"projection_{hidden_dim}"] = projection_layer.state_dict()
        
        torch.save(state_dict, save_path)
        logger.info(f"✅ 投影层参数已保存到: {save_path}")
        logger.info(f"   保存了 {len(state_dict)} 个投影层")
    ...
    def load_projection_layers(self, load_path: str):
        ...
        Args:
            load_path: 加载路径（.pth 文件）
        """
        if not Path(load_path).exists():
            logger.warning(f"投影层参数文件不存在: {load_path}")
            return False
        
        state_dict = torch.load(load_path, map_location=self.device)
        
        for key, layer_state_dict in state_dict.items():
            # key format: "projection_4096"
            hidden_dim = int(key.split("_")[1])
            
            # 如果该投影层还不存在，先创建它
            if hidden_dim not in self.z_projection_layers:
                self.create_z_projection_layer(hidden_dim)
            
            # 加载参数
            self.z_projection_layers[hidden_dim].load_state_dict(layer_state_dict)
        
        logger.info(f"✅ 投影层参数已从 {load_path} 加载")
        logger.info(f"   加载了 {len(state_dict)} 个投影层")
        return True
