...
class vLLMRollout(BaseRollout):
    ...
    def _sample_z_from_question_and_prefix(
        ...
        logger.debug(f"  pure_question: {pure_question[:50]}...")
        logger.debug(f"  prefix_text: {prefix_text[:50]}...")
        logger.debug(f"  cvae_input: {cvae_input[:100]}...")
    ...
    def _branch_single_path(
        ...
        for iteration in range(k):
            ...
            if branching_mode == "random":
                # 简化版：不使用 z，完全随机生成
                logger.debug(f"[分叉模式: random] 不使用 zi，完全随机生成")
                ...
                # 完整版：使用 PSA 注入 z
                logger.debug(f"[分叉模式: psa] 使用 PSA 注入 zi")
                # TODO: 实现 PSA 注入
                ...
                # 折中版：根据 z 调整采样参数
                logger.debug(f"[分叉模式: params] 根据 zi 调整采样参数")
                # TODO: 实现参数调整
                new_path = self._continue_from_prefix(
                    prompt_ids=prompt_ids,
                    prefix_ids=prefix_ids
            ...
            logger.debug(f"  分叉后内容: \"{new_text[:100]}...\"")
            ...
            branch_record = {
                ...
                "branching_point": {
                    ...
                    "entropy": float(max_entropy_value),
                    "prefix_text": prefix_text[:200] if len(prefix_text) > 200 else prefix_text
                },
                "zi_stats": {
                    "shape": list(z.shape),
                ...
                "original_path": {
                    "text": original_text[:300] if len(original_text) > 300 else original_text,
                    "tokens": len(original_response_ids),
                    "avg_entropy": float(original_avg_entropy)
                },
                "new_path": {
                    "text": new_text[:300] if len(new_text) > 300 else new_text,
                    "tokens": len(new_response_ids),
                    "avg_entropy": float(new_avg_entropy)
                },
                "comparison": {
                    "original_avg_entropy": float(original_avg_entropy),
                    "new_avg_entropy": float(new_avg_entropy),
                    "selected": selected
                }
            }
        ...
        initial_text = self.tokenizer.decode(initial_output.outputs[0].token_ids, skip_special_tokens=False)
        ...
        for i, path in enumerate(all_paths):
            ...
            path_text = self.tokenizer.decode(path.outputs[0].token_ids, skip_special_tokens=False)
            final_paths.append({
                "path_idx": i,
                "text": path_text[:500] if len(path_text) > 500 else path_text,
                "tokens": len(path.outputs[0].token_ids),
                "avg_entropy": float(avg_entropy),
                "is_active": (path is active_path)
            })
        
        # 构造完整记录
        branching_record = {
            "sample_idx": sample_idx,
            "pure_question": pure_question[:300] if len(pure_question) > 300 else pure_question,
            "initial_path": {
                "text": initial_text[:500] if len(initial_text) > 500 else initial_text,
                "tokens": len(initial_output.outputs[0].token_ids),
        ...
        try:
            ...
            logger.info(f"✅ 分叉数据已保存到 {branching_file}")
        except Exception as e:
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            ...
            if self.config.enable_cvae_branching and self.cvae_manager is not None:
                ...
                # 🆕 扩展 idx 和 non_tensor_batch（关键修复）
                k = self.config.cvae_num_branches_per_path
                expansion_factor = 1 + k  # 例如 k=3 时，expansion_factor=4
