...
class vLLMRollout(BaseRollout):
    ...
    def _continue_from_prefix(
        ...
        # 🔧 新增：检查长度是否超出模型限制
        max_model_len = getattr(self.inference_engine.llm_engine.model_config, 'max_model_len', 4096)
        ...
        #     stop=self.sampling_params.stop + ["<|im_end|>"],  # 🔑 加自定义停止词
        #     stop_token_ids=self.sampling_params.stop_token_ids,  # 继承 EOS IDs
        #     ignore_eos=False,  # 不忽略 EOS
        #     repetition_penalty=1.2  # 🔑 加重复惩罚，防循环
        # )
   
        # 使用提供的 sampling_params 或默认参数
        if sampling_params is None:
            # 🔧 修复：创建临时 sampling_params，继承所有原始参数（包括 stop 相关）
            print('self.sampling_params.stop_token_ids',self.sampling_params.stop_token_ids)
            temp_params = SamplingParams(
                temperature=self.sampling_params.temperature,
                top_p=self.sampling_params.top_p,
                top_k=self.sampling_params.top_k,
        ...
        logger.info(f'self.sampling_params.stop:{self.sampling_params.stop}')
    ...
    def _branch_single_path( 
        ...
        for iteration in range(k):
            ...
            if branching_mode == "random":
                ...
                new_path = self._continue_from_prefix(
                    ...
                    pure_question=pure_question  # 🔧 传入 pure_question
                )
        ...
        branching_record = {
            ...
            "pure_question": str(pure_question),  # 🔧 确保是 str 类型
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            ...
            if self.config.enable_cvae_branching and self.cvae_manager is not None:
                ...
                # 对所有初始路径进行分叉
                # idx = prompts.batch["input_ids"]  # (bs, prompt_length) idx就是 input_idx
