...
class vLLMRollout(BaseRollout):
    ...
    def _branch_single_path( 
        ...
        Returns:
            tuple: (all_paths, prefix_lengths, full_response_ids_list)
              - all_paths: List[RequestOutput] - 原始的 vLLM 输出
              - prefix_lengths: List[int] - 每个输出对应的 prefix 长度（原始路径为 0）
              - full_response_ids_list: List[List[int]] - 完整的 response (prefix + new)
        """
        ...
        # 🔧 修复 CVAE Branching Mask Bug：追踪 prefix_length 和完整的 response_ids
        prefix_lengths_list = []  # 每个路径的 prefix 长度
        full_response_ids_list = []  # 每个路径的完整 response (prefix + new)
        ...
        # 🔧 修复 CVAE Branching Mask Bug：记录初始路径的 metadata
        prefix_lengths_list.append(0)  # 原始路径没有 prefix
        full_response_ids_list.append(list(initial_response_ids))  # 完整的 response 就是原始输出
        ...
        返回类型总结
        数据集	          函数	                   返回类型
        GSM8K	gsm8k.compute_score()	float (直接返回 0, 0.0, 或 1.0)
        MATH	math.compute_score()	float (直接返回 0.0 或 1.0)
        DAPO	math_dapo.compute_score()	dict (包含 "score", "acc", "pred")
        你的逻辑完美处理了这两种情况：
        if isinstance(reward_result, dict):    # ✅ DAPO 返回字典 → 提取 "score"    initial_reward = float(reward_result.get("score", 0.0))elif isinstance(reward_result, (int, float)):    # ✅ GSM8K/MATH 返回数字 → 直接使用    initial_reward = float(reward_result)else:    # ✅ 未知类型 → 默认值    initial_reward = 0.0
        🎯 这段代码的优点
        ✅ 兼容所有数据集：能处理字典和数字两种返回类型
        ✅ 异常安全：外层有 try-except，内层有类型检查
        ...
        if gt is not None and data_source is not None:
            try:
                reward_result = default_compute_score(
                ...
                if isinstance(reward_result, dict):
                    # 如果返回字典（例如 math_dapo），提取 "score" 字段
                    initial_reward = float(reward_result.get("score", 0.0))
                    logger.debug(f"[初始分支 {initial_branch_name}] reward dict: {reward_result}, score: {initial_reward}")
                elif isinstance(reward_result, (int, float)):
                    # 如果返回数字，直接使用
                    initial_reward = float(reward_result)
                    logger.debug(f"[初始分支 {initial_branch_name}] reward: {initial_reward}")
                else:
                    # 其他情况，使用默认值
                    logger.warning(f"[初始分支 {initial_branch_name}] reward 返回未知类型: {type(reward_result)}, 使用默认值 0.0")
                    initial_reward = 0.0
            except Exception as e:
        ...
        for iteration in range(k):
            ...
            # 8. 生成新分支名称（a1, a2, a3...） - 🔧 提前定义，避免 UnboundLocalError
            new_branch_name = f"{base_letter}{iteration + 1}"
            
            # 9. 添加到路径列表
            all_paths.append(new_path)
            
            # 🔧 修复 CVAE Branching Mask Bug：记录分叉路径的 metadata
            # prefix_length = 前缀的长度（从父路径截取到分叉点）
            # full_response_ids = 完整的 response (prefix + new)
            prefix_length = len(parent_prefix_ids)
            full_response_ids = parent_prefix_ids + new_response_ids
            prefix_lengths_list.append(prefix_length)
            full_response_ids_list.append(full_response_ids)
            
            logger.debug(f"  [MASK FIX] 分支 {new_branch_name}: prefix_len={prefix_length}, full_len={len(full_response_ids)}, new_len={len(new_response_ids)}")
            
            # 10. 计算平均熵
            # active_path 是分叉前的 老路，因为比拼 其他的entropy它被激活 
            ...
            if gt is not None and data_source is not None:
                try:
                    ...
                    if isinstance(reward_result, dict):
                        # 如果返回字典（例如 math_dapo），提取 "score" 字段
                        new_reward = float(reward_result.get("score", 0.0))
                        logger.debug(f"[新分支 {new_branch_name}] reward dict: {reward_result}, score: {new_reward}")
                    elif isinstance(reward_result, (int, float)):
                        # 如果返回数字，直接使用
                        new_reward = float(reward_result)
                        logger.debug(f"[新分支 {new_branch_name}] reward: {new_reward}")
                    else:
                        # 其他情况，使用默认值
                        logger.warning(f"[新分支 {new_branch_name}] reward 返回未知类型: {type(reward_result)}, 使用默认值 0.0")
                        new_reward = 0.0
                except Exception as e:
        ...
        # 🔧 修复 CVAE Branching Mask Bug：返回完整的 metadata
        logger.debug(f"[_branch_single_path] 返回 {len(all_paths)} 个路径，prefix_lengths: {prefix_lengths_list}")
        return all_paths, prefix_lengths_list, full_response_ids_list
    ...
    def _perform_branching_for_all_paths(
        ...
        Returns:
            tuple: (all_branched_paths, all_prefix_lengths, all_full_response_ids)
              - all_branched_paths: List[RequestOutput] - batch_size × rollout_n × (1 + k) 条路径
              - all_prefix_lengths: List[int] - 每个路径的 prefix 长度
              - all_full_response_ids: List[List[int]] - 每个路径的完整 response (prefix + new)
        """
        ...
        all_branched_paths = []
        # 🔧 修复 CVAE Branching Mask Bug：初始化三个总列表
        all_prefix_lengths = []
        all_full_response_ids = []
        ...
        for prompt_idx, request_output in enumerate(initial_outputs):
            ...
            for sample_idx_in_prompt, completion_output in enumerate(request_output.outputs):
                ...
                # 对单条路径进行分叉
                # 🔧 修复 CVAE Branching Mask Bug：解包返回的三个列表
                branched_paths, prefix_lengths, full_response_ids = self._branch_single_path(
                ...
                all_full_response_ids.extend(full_response_ids)
        ...
        # 🔧 修复 CVAE Branching Mask Bug：返回完整的 metadata
        logger.debug(f"[_perform_branching_for_all_paths] 返回 {len(all_branched_paths)} 个路径")
        logger.debug(f"  prefix_lengths 长度: {len(all_prefix_lengths)}")
        logger.debug(f"  full_response_ids 长度: {len(all_full_response_ids)}")
        return all_branched_paths, all_prefix_lengths, all_full_response_ids
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            ...
            # 🆕 CVAE 分叉逻辑
            # 🔧 修复 CVAE Branching Mask Bug：初始化 prefix_lengths 和 full_response_ids_list
            prefix_lengths = None
            full_response_ids_list = None
            ...
            if self.config.enable_cvae_branching and self.cvae_manager is not None:
                ...
                # 对所有初始路径进行分叉
                # 🔧 修复 CVAE Branching Mask Bug：解包三个返回值
                outputs, prefix_lengths, full_response_ids_list = self._perform_branching_for_all_paths(
                    initial_outputs=outputs,
                    idx=idx,
                    non_tensor_batch=non_tensor_batch,
                    prompts=prompts  # 🆕 传递 prompts 以获取 epoch
                )
                logger.info(f"[MASK FIX] 获取到 {len(prefix_lengths)} 个 prefix_length, {len(full_response_ids_list)} 个 full_response_ids")
            ...
            # 🔧 修复 CVAE Branching Mask Bug：追踪当前处理的路径索引
            path_idx = 0
            
            for output in outputs:
                for sample_id in range(len(output.outputs)):
                    # 🔧 修复 CVAE Branching Mask Bug：使用 full_response_ids 或默认的 token_ids
                    if full_response_ids_list is not None and path_idx < len(full_response_ids_list):
                        # Branching 情况：使用完整的 response (prefix + new)
                        response_ids = full_response_ids_list[path_idx]
                        logger.debug(f"[MASK FIX] 路径 {path_idx}: 使用 full_response_ids, len={len(response_ids)}")
                    else:
                        # 正常情况：直接使用 vLLM 输出
                    ...
                    if self.config.calculate_log_probs:
                        # ⚠️ 注意：log_probs 只对应新生成的部分，不包括 prefix
                        # 方案A：训练时会重新计算，所以这里可以忽略或填充 0
                        if full_response_ids_list is not None and prefix_lengths is not None and path_idx < len(prefix_lengths):
                            prefix_len = prefix_lengths[path_idx]
                            # 为 prefix 部分填充 0（因为 mask=0 时不会用到）
                            curr_log_prob = [0.0] * prefix_len
                            # 添加新生成部分的 log_probs
                            if output.outputs[sample_id].logprobs:
                                for i, logprob in enumerate(output.outputs[sample_id].logprobs):
                                    curr_log_prob.append(logprob[output.outputs[sample_id].token_ids[i]].logprob)
                        else:
                            # 正常情况：直接使用 vLLM 的 log_probs
                            curr_log_prob = []
                            for i, logprob in enumerate(output.outputs[sample_id].logprobs):
                                curr_log_prob.append(logprob[response_ids[i]].logprob)
                        rollout_log_probs.append(curr_log_prob)
                    
                    # 🔧 修复：提取熵值，并为 prefix 填充 0（与 log_probs 逻辑一致）
                    if hasattr(output.outputs[sample_id], 'entropies') and output.outputs[sample_id].entropies:
                        if full_response_ids_list is not None and prefix_lengths is not None and path_idx < len(prefix_lengths):
                            prefix_len = prefix_lengths[path_idx]
                            # 为 prefix 部分填充 0.0（prefix 不参与训练，熵值无意义）
                            curr_entropy = [0.0] * prefix_len
                            # 添加新生成部分的 entropies
                            curr_entropy.extend(list(output.outputs[sample_id].entropies))
                            rollout_entropies.append(curr_entropy)
                            
                            # 🔍 DEBUG: 验证长度对齐（仅前3个样本）
                            if path_idx < 3:
                                logger.info(f"[Entropy Fix] Sample {path_idx}: response_len={len(response_ids)}, "
                                           f"prefix_len={prefix_len}, entropy_len={len(curr_entropy)}, "
                                           f"new_entropy_len={len(output.outputs[sample_id].entropies)}")
                        else:
                            # 正常情况：直接使用 vLLM 的 entropies
                            rollout_entropies.append(list(output.outputs[sample_id].entropies))
                    else:
                        # 如果没有 entropies，填充空列表或零（确保长度一致）
                        if full_response_ids_list is not None and path_idx < len(full_response_ids_list):
                            rollout_entropies.append([0.0] * len(response_ids))
                            logger.warning(f"[Entropy Fix] Sample {path_idx}: No entropies from vLLM, filled with zeros")
                        else:
                            rollout_entropies.append([])
        ...
        # 🔧 修复 CVAE Branching Mask Bug：构建自定义的 response_mask（用于 loss 计算）
        custom_response_masks = []
        if prefix_lengths is not None and full_response_ids_list is not None:
            logger.info(f"[MASK FIX] 构建自定义 response_mask（CVAE Branching 模式）")
            for i, response_ids in enumerate(response):
                prefix_len = prefix_lengths[i] if i < len(prefix_lengths) else 0
                response_len = len(response_ids)
                
                # 构建 mask：prefix=0, new=1
                if prefix_len > 0:
                    mask_i = [0] * prefix_len + [1] * (response_len - prefix_len)
                else:
                    mask_i = [1] * response_len
                
                custom_response_masks.append(mask_i)
                
                if i < 3:  # 只打印前3个样本
                    # 🆕 显式验证：Prefix 部分 Mask 必须全为 0
                    prefix_mask_segment = mask_i[:prefix_len]
                    is_all_zero = all(m == 0 for m in prefix_mask_segment)
                    status_icon = "✅" if is_all_zero else "❌"
                    
                    logger.info(f"  🔍 [Mask Check] Sample {i}: Prefix_len={prefix_len} -> Mask段全是0? {status_icon}")
                    if prefix_len > 0:
                        # 打印 mask 的前几个值和分界点附近的值
                        boundary_start = max(0, prefix_len - 3)
                        boundary_end = min(len(mask_i), prefix_len + 3)
                        logger.info(f"      Mask边界 [{boundary_start}:{boundary_end}]: {mask_i[boundary_start:boundary_end]} (分界点在 {prefix_len})")
        else:
            logger.debug(f"[MASK FIX] 使用默认 response_mask（无 CVAE Branching）")
            # 正常情况：全部是 1
            for response_ids in response:
                mask_i = [1] * len(response_ids)
                custom_response_masks.append(mask_i)
        
        # Pad custom masks 到统一长度
        # 注意：pad_2d_list_to_length 已在顶部导入（第 62 行）
        custom_response_masks_tensor = pad_2d_list_to_length(
            custom_response_masks, 0,  # padding的mask也是0（不参与loss）
            max_length=self.config.response_length
        ).to(idx.device)
        
        logger.info(f"[MASK FIX] custom_response_masks_tensor shape: {custom_response_masks_tensor.shape}")
        ...
        batch = TensorDict(
            {
                ...
                "response_mask": custom_response_masks_tensor,  # 🔧 添加自定义的 response_mask
            },
