...
class ActorConfig(BaseConfig):
    ...
    Args:
        ...
        cvae_projection_lr (Optional[float]): Learning rate for CVAE projection layers. 
            If None, defaults to optim.lr * 0.1.
        use_ib_regularization (bool): Whether to enable IB (Information Bottleneck) regularization.
            When enabled, entropy is weighted by token-level advantages before aggregation.
    """
    ...
    # 🆕 CVAE 投影层训练配置
    cvae_projection_lr: Optional[float] = None  # CVAE 投影层学习率（如果为 None，使用 optim.lr * 0.1）
    # 🆕 IB Regularization 配置
    use_ib_regularization: bool = False  # 是否启用 IB Regularization（用 advantages 加权 entropy）
...
class McoreActorConfig(ActorConfig):
    ...
    # 🔧 显式在子类中重新定义
    cvae_projection_lr: Optional[float] = None
    use_ib_regularization: bool = False
