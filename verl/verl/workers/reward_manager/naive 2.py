    ...
    def __call__(self, data: DataProto, return_dict: bool = False) -> torch.Tensor | dict[str, Any]:
        ...
        for i in range(len(data)):
            ...
            if already_print_data_sources[data_source] < self.num_examine:
                ...
                print("[ground_truth]", ground_truth[:100] + "..." if isinstance(ground_truth, str) and len(ground_truth) > 100 else ground_truth)
                if isinstance(score, dict):
                    for key, value in score.items():
                        print(f"[{key}]", value)
                else:
                ...
                try:
                    ...
                    # 🔧 正确处理 ground_truth（可能是 numpy array/scalar）
                    gt_str = None
                    if ground_truth is not None:
                        if hasattr(ground_truth, 'item'):  # numpy scalar
                            gt_str = str(ground_truth.item())
                        elif isinstance(ground_truth, bytes):
                            gt_str = ground_truth.decode('utf-8')
                        else:
