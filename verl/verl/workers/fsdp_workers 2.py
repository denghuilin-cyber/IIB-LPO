        ...
        if self._is_rollout:
            ...
            if self._is_actor and hasattr(self, 'rollout') and hasattr(self.rollout, 'cvae_manager') and self.rollout.cvae_manager is not None:
                ...
                if len(cvae_projection_params) > 0:
                    # 🔧 优先从 actor_cfg 读取 cvae_projection_lr，其次从 optim_config，最后使用默认值
                    cvae_projection_lr = actor_cfg.get("cvae_projection_lr", 
                                                       optim_config.get("cvae_projection_lr", 
                                                                       optim_config.lr * 0.1))  # 默认使用 actor lr 的 0.1
