...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            # 🆕 先不传递 cvae_projection_optimizer，之后在 rollout 构建后再设置
            self.actor = DataParallelPPOActor(
                config=actor_cfg, 
                actor_module=self.actor_module_fsdp, 
                actor_optimizer=self.actor_optimizer,
                cvae_projection_optimizer=None  # 暂时为 None，稍后设置
            )

        if self._is_rollout:
            self._build_rollout(trust_remote_code=self.config.model.get("trust_remote_code", False))
            # 🆕 在 rollout 构建后，为 actor 创建 CVAE 投影层的 optimizer
            if self._is_actor and hasattr(self, 'rollout') and hasattr(self.rollout, 'cvae_manager') and self.rollout.cvae_manager is not None:
                # 设置投影层为可训练
                self.rollout.cvae_manager.set_projection_layers_trainable(True)
                
                cvae_projection_params = self.rollout.cvae_manager.get_projection_parameters()
                if len(cvae_projection_params) > 0:
                    # 🔧 从 actor_cfg 读取 cvae_projection_lr（如果为 None，使用默认值）
                    cvae_projection_lr = actor_cfg.cvae_projection_lr
                    if cvae_projection_lr is None:
                        cvae_projection_lr = optim_config.lr * 0.1  # 默认使用 actor lr 的 0.1
                    cvae_projection_optimizer = optim.AdamW(
                        cvae_projection_params,
                        lr=cvae_projection_lr,
                        betas=optim_config.get("betas", (0.9, 0.999)),
                        weight_decay=optim_config.get("weight_decay", 1e-2),
                    )
                    
                    # 🆕 设置到 actor 中
                    self.actor.cvae_projection_optimizer = cvae_projection_optimizer
                    
                    if self.rank == 0:
                        total_params = sum(p.numel() for p in cvae_projection_params)
                        print(f"✅ CVAE projection optimizer created:")
                        print(f"   - Learning rate: {cvae_projection_lr}")
                        print(f"   - Total parameters: {total_params:,}")
                        print(f"   - Number of projection layers: {len(self.rollout.cvae_manager.get_projection_layers_dict())}")
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_cvae_projection_checkpoint(self, global_step_folder, global_step, max_ckpt_to_keep=None):
        """
        保存 CVAE 投影层 checkpoint 到独立文件夹
        
        Args:
            global_step_folder: global_step_{step} 文件夹路径
            global_step: 当前训练步数
            max_ckpt_to_keep: 最多保留的 checkpoint 数量
        """
        # 只有 actor 且启用了 CVAE 时才保存
        if not self._is_actor:
            return
        
        if not (hasattr(self, 'rollout') and hasattr(self.rollout, 'cvae_manager') and self.rollout.cvae_manager is not None):
            return
        
        if len(self.rollout.cvae_manager.get_projection_layers_dict()) == 0:
            return
        
        # 创建 cvae_projection 文件夹（与 actor、critic 平级）
        cvae_projection_folder = os.path.join(global_step_folder, "cvae_projection")
        
        if dist.get_rank() == 0:
            os.makedirs(cvae_projection_folder, exist_ok=True)
            
            # 保存投影层参数
            cvae_projection_save_path = os.path.join(cvae_projection_folder, "projection_layers.pth")
            self.rollout.cvae_manager.save_projection_layers(cvae_projection_save_path)
            logger.info(f"✅ Saved CVAE projection layers to: {cvae_projection_save_path}")
            
            # 🆕 实现 max_ckpt_to_keep 清理策略
            if max_ckpt_to_keep is not None and max_ckpt_to_keep > 0:
                # 获取所有 global_step_* 文件夹
                parent_dir = os.path.dirname(global_step_folder)
                all_step_folders = sorted(
                    [d for d in os.listdir(parent_dir) if d.startswith("global_step_")],
                    key=lambda x: int(x.split("_")[-1])
                )
                
                # 删除旧的 CVAE checkpoint
                if len(all_step_folders) > max_ckpt_to_keep:
                    folders_to_remove = all_step_folders[:-max_ckpt_to_keep]
                    for folder in folders_to_remove:
                        cvae_folder_to_remove = os.path.join(parent_dir, folder, "cvae_projection")
                        if os.path.exists(cvae_folder_to_remove):
                            import shutil
                            shutil.rmtree(cvae_folder_to_remove)
                            logger.info(f"🗑️  Removed old CVAE checkpoint: {cvae_folder_to_remove}")
    ...
    def load_checkpoint(self, local_path, hdfs_path=None, del_local_after_load=False):
        ...
        # 🆕 加载 CVAE 投影层参数（从 global_step_folder/cvae_projection/ 加载）
        if self._is_actor and hasattr(self, 'rollout') and hasattr(self.rollout, 'cvae_manager') and self.rollout.cvae_manager is not None:
            # local_path 是 global_step_{step}/actor，需要回到 global_step_{step} 文件夹
            global_step_folder = os.path.dirname(local_path)
            cvae_projection_folder = os.path.join(global_step_folder, "cvae_projection")
            cvae_projection_file = os.path.join(cvae_projection_folder, "projection_layers.pth")
            
            if os.path.exists(cvae_projection_file):
                if dist.get_rank() == 0:
                    logger.info(f"Loading CVAE projection layers from: {cvae_projection_file}")
                self.rollout.cvae_manager.load_projection_layers(cvae_projection_file)
            else:
                if dist.get_rank() == 0:
                    logger.warning(f"No CVAE projection checkpoint found at {cvae_projection_file}, using random initialization")
            # 🔧 修复死锁：无论文件是否存在，都要 barrier
            dist.barrier()
