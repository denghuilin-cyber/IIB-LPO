...
class RayPPOTrainer:
    ...
    def _init_cot_augmenter(self):
        ...
        # 🆕 获取 max_prompt_length（用于多层降级策略）
        # 优先从 data.max_prompt_length 获取，如果没有则从 rollout.prompt_length 获取
        max_prompt_length = None
        try:
            # 尝试从 data.max_prompt_length 获取
            if hasattr(self.config, 'data') and self.config.data is not None:
                if hasattr(self.config.data, 'max_prompt_length'):
                    max_prompt_length = self.config.data.max_prompt_length
                elif hasattr(self.config.data, 'get'):
                    max_prompt_length = self.config.data.get('max_prompt_length', None)
        except Exception:
            pass
        
        # 如果还没有获取到，尝试从 rollout.prompt_length 获取
        if max_prompt_length is None:
            try:
                if hasattr(self.config.actor_rollout_ref, 'rollout') and self.config.actor_rollout_ref.rollout is not None:
                    if hasattr(self.config.actor_rollout_ref.rollout, 'prompt_length'):
                        max_prompt_length = self.config.actor_rollout_ref.rollout.prompt_length
                    elif hasattr(self.config.actor_rollout_ref.rollout, 'get'):
                        max_prompt_length = self.config.actor_rollout_ref.rollout.get('prompt_length', None)
            except Exception:
                pass
        ...
        if max_prompt_length is None:
            ...
            print(f"⚠️  Warning: max_prompt_length not found in config, using default value: {max_prompt_length}")
        else:
        ...
        self.cot_augmenter = GRPOCOTAugmenter(
            ...
            max_prompt_length=max_prompt_length,  # 🆕 传递 max_prompt_length 参数
        )
        print(f"COT augmenter initialized with strategy: {cot_config.get('sampling_strategy', 'sequential')}, max_prompt_length: {max_prompt_length}")
