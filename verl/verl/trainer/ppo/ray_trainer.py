...
class RayPPOTrainer:
    ...
    def _init_cot_augmenter(self):
        ...
        self.cot_augmenter = GRPOCOTAugmenter(
            ...
            max_prompt_length=self.config.data.max_prompt_length,  # 🔧 修复：从配置中读取 max_prompt_length
        )
...
# 替换函数: RayPPOTrainer._validate
    ...
    def _save_checkpoint(self):
        ...
        # 🆕 保存 CVAE 投影层（使用与 actor 相同的 max_ckpt_to_keep 策略）
        max_cvae_ckpt_to_keep = self.config.trainer.get("max_cvae_ckpt_to_keep", max_actor_ckpt_to_keep)
        self.actor_rollout_wg.save_cvae_projection_checkpoint(
            local_global_step_folder, self.global_steps, max_ckpt_to_keep=max_cvae_ckpt_to_keep
            )
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if reward_extra_infos_dict:
                            # 🔧 修复：只添加非空且长度匹配的 reward_extra_info，避免 chunk 时出现长度不匹配
                            batch_size = len(batch)
                            non_empty_reward_extra_infos = {}
                            for k, v in reward_extra_infos_dict.items():
                                if len(v) == batch_size:
                                    non_empty_reward_extra_infos[k] = np.array(v, dtype=object)
                                else:
                                    # 🔧 使用 logging.getLogger 而不是 logger（logger 是 Tracking 对象）
                                    logging.getLogger(__name__).warning(f"⚠️  Skipping reward_extra_info key '{k}' due to length mismatch: expected {batch_size}, got {len(v)}")
                            if non_empty_reward_extra_infos:
                                batch.non_tensor_batch.update(non_empty_reward_extra_infos)
                    ...
                    if rollout_data_dir:
                        ...
                        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
                            ...
                            # reward_model 是 numpy array of dicts (由 collate_fn 产生)
                            sample_gts = []
                            if "reward_model" in batch.non_tensor_batch:
                                reward_model_arr = batch.non_tensor_batch["reward_model"]
                                
                                # 遍历每个样本的 reward_model
                                for i in range(len(inputs)):
                                    if isinstance(reward_model_arr, np.ndarray) and i < len(reward_model_arr):
                                        reward_model_entry = reward_model_arr[i]
                                        if isinstance(reward_model_entry, dict):
                                            gt = reward_model_entry.get("ground_truth", None)
                                            # 🔧 确保 ground_truth 是可序列化的字符串
                                            if gt is not None:
