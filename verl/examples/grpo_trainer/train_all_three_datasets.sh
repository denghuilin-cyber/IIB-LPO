#!/bin/bash
# 🔥 版本 3: 训练 MATH + GSM8K + DAPO 三个数据集
# 特点：MATH 和 GSM8K 使用 COT 增强，DAPO 自动跳过（skip_on_mismatch=true）
...
# 输出目录
OUTPUT_DIR="/nas/dhl/outputs/train_all_three_$(date +%Y%m%d_%H%M%S)"
mkdir -p "${OUTPUT_DIR}"
...
echo "================================"
echo "🎯 训练配置: MATH + GSM8K + DAPO"
echo "================================"
echo "数据集 1: GSM8K"
echo "  数据: $TRAIN_FILES_GSM8K"
echo "  COT: $GSM8K_COT ✅"
echo ""
echo "数据集 2: MATH"
...
python3 -m verl.trainer.main_ppo \
  ...
  trainer.project_name=train_all_three \
  trainer.experiment_name=gsm8k_math_dapo \
