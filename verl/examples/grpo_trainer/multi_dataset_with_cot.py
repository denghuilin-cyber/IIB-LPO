...
class MultiDatasetWithCOT(Dataset):
    ...
    def __init__(
        ...
        dapo_path: Optional[str] = None,  # 🆕 添加 DAPO
        ...
        # 🔧 配置 chat_template
        # 因为第60行已经确保 self.config 不为 None（最坏是空字典），所以直接使用
        self.chat_template = self.config.get("chat_template", "qwen2.5")
        
        # 定义多个 prompt 模板（手工构建，包含所有特殊 token）
        if self.chat_template == "qwen2.5":
            self.prompt_template = "<|im_start|>system\nYou are a helpful assistant.<|im_end|>\n<|im_start|>user\n{input}\nPlease reason step by step, and put your final answer within \\boxed{}.<|im_end|>\n<|im_start|>assistant\n"
        elif self.chat_template == "qwen3":
            self.prompt_template = "<|im_start|>system\nYou are a helpful assistant.<|im_end|>\n<|im_start|>user\n{input}\nPlease reason step by step, and put your final answer within \\boxed{}.<|im_end|>\n<|im_start|>assistant\n<think>\n\n</think>\n"
        elif self.chat_template == "llama":
            self.prompt_template = "<|begin_of_text|><|start_header_id|>system<|end_header_id|>You are a helpful assistant.<|eot_id|><|start_header_id|>user<|end_header_id|>{input}\nPlease reason step by step, and put your final answer within \\boxed{}.<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n"
        else:
            # 默认使用 qwen2.5 格式
            self.prompt_template = "<|im_start|>system\nYou are a helpful assistant.<|im_end|>\n<|im_start|>user\n{input}\nPlease reason step by step, and put your final answer within \\boxed{}.<|im_end|>\n<|im_start|>assistant\n"
        
        print(f"📝 使用 chat_template: {self.chat_template}")
        
        # 🆕 从config中读取简化路径和字段名（优先级最高）
        # 初始化默认值
        _gsm8k_path, _math_path, _numina_path, _dapo_path = None, None, None, None
        ...
        if config is not None:
            ...
            if is_train:
                ...
                _dapo_path = dapo_path or config.get('dapo_path')  # 🔧 修复：使用 dapo_path 参数
            ...
            dapo_prompt_key = config.get('dapo_prompt_key', 'prompt')  # 🆕 DAPO
            dapo_answer_key = config.get('dapo_answer_key', 'reward_model.ground_truth')  # 🆕 DAPO
        else:
            # 没有 config 时的默认字段名
            ...
            dapo_prompt_key = 'prompt'  # 🆕 DAPO
            dapo_answer_key = 'reward_model.ground_truth'  # 🆕 DAPO
        ...
        if self.is_train and not dataset_configs_list and (_gsm8k_path or _math_path or _numina_path or _dapo_path):
            if _gsm8k_path:
            ...
            if _dapo_path:  # 🆕 添加 DAPO
                dataset_configs_list.append({
                    'name': 'dapo', 'files': [_dapo_path],
                    'prompt_key': dapo_prompt_key, 'answer_key': dapo_answer_key
                })
        ...
        if not dataset_configs_list and is_train:
             raise ValueError(
                "训练集必须提供 dataset_configs 或至少一个数据集路径（gsm8k_path/math_path/numina_path/dapo_path）。"
            )
        ...
        for dataset_config in self.dataset_configs:
            ...
            for idx, row in dataset_df.iterrows():
                ...
                if prompt_key == 'prompt' and 'prompt' in row:
                    ...
                    # 🔧 支持 list 和 numpy.ndarray
                    if isinstance(prompt_value, (list, np.ndarray)):
                        # Chat格式: [{'content': '...', 'role': 'user'}]
                        if len(prompt_value) > 0 and isinstance(prompt_value[0], dict) and 'content' in prompt_value[0]:
                ...
                # 🔧 在数据加载时就删除冲突的提示语（针对 DAPO 等数据集）
                # 这样可以确保存储的 question 是干净的
                ...
                for instruction in conflicting_instructions:
                    if instruction in question:
                        ...
                        logger.debug(f"[数据加载] 删除冲突提示语: {instruction[:50]}...")
                    if instruction in question_for_matching:
                        question_for_matching = question_for_matching.replace(instruction, "").strip()
    ...
    def __getitem__(self, index):
        ...
        # 清理冲突的提示语（数据集自带的）
        # 某些数据集的 question 中包含格式指令，这与我们的模板冲突，需要删除
        conflicting_instructions = [
            # boxed 格式指令
        ...
        # 🔑 使用硬编码模板生成 prompt（和龙哥完全一致）
        # 完全绕过 apply_chat_template()，直接字符串替换
        # 模板中已包含：system prompt + 特殊 token + 指令 + assistant 开始标记
        prompt_text = self.prompt_template.replace("{input}", question)
        ...
        # 🔧 处理 question_id：尝试转换为 int，如果失败则保持为字符串（例如 DAPO 的 UUID）
        try:
            result['question_id'] = int(question_id)
        except (ValueError, TypeError):
            result['question_id'] = str(question_id)
        ...
        elif 'math_dapo' in original_data_source.lower() or dataset_name == 'dapo':
            result['data_source'] = 'math_dapo'  # 🆕 DAPO 使用 math_dapo（verl 已支持）
        
        # 🆕 添加完整的 prompt（用于 DAPO 数据集在 branching.jsonl 中的显示）
        result['full_prompts'] = prompt_text
