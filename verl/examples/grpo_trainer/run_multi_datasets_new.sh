...
VAL_FILES="/nas/dhl/Datasets/my_Datasets/gsm8k/test.parquet"
...
# 输出
OUTPUT_DIR="/nas/dhl/outputs/gsm8k_math_grpo_cot"

python3 -m verl.trainer.main_ppo \
  ++data.custom_cls.path=examples.grpo_trainer.multi_dataset_with_cot \
  ++data.custom_cls.name=MultiDatasetWithCOT \
  ++data.dataset_configs="[{'name':'gsm8k','files':['${TRAIN_FILES_GSM8K}'],'prompt_key':'question','answer_key':'answer'},{'name':'math','files':['${TRAIN_FILES_MATH}'],'prompt_key':'question','answer_key':'answer'}]" \
  \
