    ...
    def _branch_single_path( 
        ...
        for iteration in range(k):
            ...
            if branching_mode == "random":
                ...
                # 模式2：INPUT Fusion - 将 z 投影后加到第一层的输入 hidden states
                logger.info(f"[分叉模式: input] 使用 INPUT Fusion 注入 zi")
                llm_model = self._get_llm_model()
                self.cvae_manager.register_input_fusion_hook(
                    model=llm_model,
                    z=z
                )
                try:
                ...
                # 模式3：PSA Fusion - 在指定层的 hidden states 上添加 z（近似方案）
                logger.info(f"[分叉模式: psa] 🎯 使用 PSA Fusion 注入 zi 到指定层")
                logger.info(f"  z 形状: {z.shape}")
                llm_model = self._get_llm_model()
                logger.info(f"  LLM 模型类型: {type(llm_model)}")
                self.cvae_manager.register_psa_fusion_hooks(
                    model=llm_model,
                    z=z
                )
                try:
                    ...
                    logger.info(f"  生成完成，移除 hooks...")
                finally:
                    self.cvae_manager.remove_hooks()
                    logger.info(f"  Hooks 已移除")
            
            elif branching_mode == "softmax":
                # 模式4：SOFTMAX Fusion - 使用 ForwardContext 在 softmax 前修改 logits
                logger.info(f"[分叉模式: softmax] 🎯 使用 SOFTMAX Fusion (ForwardContext) 注入 zi")
                logger.info(f"  z 形状: {z.shape}")
                ...
                # 投影 z 到词表空间
                z_proj_vocab = self.cvae_manager.project_z_to_vocab(
                    z=z,
                    vocab_size=vocab_size
                )
                
                # 创建 ZInjectionConfig
                from vllm.forward_context import ZInjectionConfig
                z_injection_config = ZInjectionConfig(
                    mode="softmax",
