...
class SequenceData(msgspec.Struct,
    ...
    # 🆕 新增字段：存储每个输出 token 的熵值
    # 与 _output_token_ids 一一对应
    _output_entropies: list[float] = msgspec.field(default_factory=list)
    ...
    def append_token_id(self, token_id: int, logprob: float, entropy: Optional[float] = None) -> None:  # 🆕 添加 entropy 参数
        ...
        if entropy is not None:
            self._output_entropies.append(entropy)
            if len(self._output_entropies) == 1:  # 只在第一个 token 时打印
                print(f"[SEQUENCE DEBUG] 🎯 存储熵值: token_id={token_id}, entropy={entropy:.4f}, total_entropies={len(self._output_entropies)}")
    ...
    # 🆕 新增方法：获取输出 token 的熵值列表
    def get_output_entropies(self) -> list[float]:
        """Return the entropy values of the output tokens.
        
        Returns:
            A list of entropy values, one for each output token.
            The length should match get_output_len().
        """
        return self._output_entropies
...
class Sequence:
    ...
    def append_token_id(self, token_id: int, logprobs: dict[int, Logprob], 
                        entropy: Optional[float] = None) -> None:  # 🆕 添加 entropy 参数
        assert token_id in logprobs
        self.output_logprobs.append(logprobs)
        self.data.append_token_id(token_id, logprobs[token_id].logprob, entropy=entropy)  # 🆕 传递 entropy
...
class SequenceOutput(
    ...
    Args:
        ...
        entropy: 🆕 The entropy value for this output token.
    """
    parent_seq_id: int
    output_token: int
    logprobs: dict[int, Logprob]
    entropy: Optional[float] = None  # 🆕 新增字段

    def __repr__(self) -> str:
        return (f"SequenceOutput(parent_seq_id={self.parent_seq_id}, "
                f"output_token={self.output_token}, "
                f"logprobs={self.logprobs}, "
                f"entropy={self.entropy})")  # 🆕 添加
