...
class ZInjectionConfig:
    ...
    - 任何时刻只能有一个模式生效
    """
    mode: Optional[str] = None  # "input", "softmax" (psa 和 random 不使用 ForwardContext)
    z_proj_vocab: Optional[torch.Tensor] = None  # For SOFTMAX mode: [vocab_size]
    z_proj_input: Optional[torch.Tensor] = None  # For INPUT mode: [1, hidden_size]
...
def validate_z_injection_config(config: Optional['ZInjectionConfig']) -> bool:
    ...
    # 规则1: mode 必须是合法值
    if mode not in ["input", "softmax", None]:
        raise ValueError(
            f"❌ 非法的 mode: {mode}\n"
            f"   合法值: 'input', 'softmax', None\n"
            f"   注意: 'psa' 和 'random' 模式不使用 ForwardContext"
        )
    ...
    if mode == "input":
        ...
        if z_proj_vocab is not None:
            raise ValueError(
                "❌ mode='input' 时不允许设置 z_proj_vocab\n"
                "   INPUT Fusion 和 SOFTMAX Fusion 是互斥的！"
            )
    
    # 规则3: mode="softmax" 时，只能有 z_proj_vocab
    if mode == "softmax":
        if z_proj_vocab is None:
            raise ValueError("❌ mode='softmax' 但 z_proj_vocab 为 None")
        if z_proj_input is not None:
            raise ValueError(
                "❌ mode='softmax' 时不允许设置 z_proj_input\n"
                "   SOFTMAX Fusion 和 INPUT Fusion 是互斥的！"
            )
    
    # 规则4: mode=None 时，两个都不能有值
    if mode is None:
