...
class Scheduler(SchedulerInterface):
    ...
    def update_from_output(
        ...
        # 🆕 提取熵值（如果有）
        entropies_per_req = model_runner_output.entropies or []
        ...
        for request in self.running:
            ...
            # 🆕 提取当前请求的熵值
            new_entropies = None
            if entropies_per_req and req_index < len(entropies_per_req):
                new_entropies = entropies_per_req[req_index]
            ...
            for num_new, output_token_id in enumerate(new_token_ids, 1):
                # 🆕 提取当前 token 的熵值
                token_entropy = None
                if new_entropies and num_new - 1 < len(new_entropies):
                    token_entropy = new_entropies[num_new - 1]
                
                request.append_output_token_ids(output_token_id, entropies=token_entropy)
            ...
            if new_token_ids:
                ...
                outputs.append(
                    EngineCoreOutput(
                        ...
                        new_entropies=new_entropies))  # 🆕 添加熵值
            else:
