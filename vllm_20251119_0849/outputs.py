...
class CompletionOutput:
    ...
    Args:
        ...
        entropies: 🆕 The entropy value for each generated token. The length
            should match the length of token_ids. Each value represents
            H = -sum(p * log(p)) where p is the probability distribution
            over the vocabulary at that token position.
    """
    ...
    entropies: Optional[list[float]] = None  # 🆕 新增字段
    ...
    def from_seq_group(
        ...
        for i, seq in enumerate(top_n_seqs):
            ...
            if use_cache:
                ...
                # 🆕 更新熵值
                output.entropies = seq.data.get_output_entropies() if hasattr(seq.data, 'get_output_entropies') else None

            else:
                # 🆕 获取熵值列表
                output_entropies = seq.data.get_output_entropies() if hasattr(seq.data, 'get_output_entropies') else None
                if output_entropies and len(output_entropies) > 0:
                    print(f"[OUTPUTS DEBUG] 🎯 CompletionOutput 包含熵值: len={len(output_entropies)}, 前3个值={output_entropies[:3]}")
                else:
                    print(f"[OUTPUTS DEBUG] ⚠️  CompletionOutput 熵值为空: output_entropies={output_entropies}")
                ...
                output = CompletionOutput(
                    ...
                    seq.stop_reason,
                    entropies=output_entropies)  # 🆕 添加熵值
